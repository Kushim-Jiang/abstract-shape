...
names_dict = {"main": "main", "ExtA": "a", "ExtB": "b"}


def build_historical():
    for sheet_name, file_name in names_dict.items():
        result = ""
        ...
        with open(input_dir / f"history_{file_name}.txt", "w", encoding="utf-8") as f:
            f.write(result)
        ...
        for line in lines[1:]:
            try:
                ...
                if (abst + reco + comm).strip() != "":
                    store_char = char if char.strip() != "" else store_char
                    result += f"{store_char}\t{abst}\t{reco}\t{comm}".rstrip() + "\n"
            except ValueError:
                pass

        with open(input_dir / f"abstract_{file_name}.txt", "w", encoding="utf-8") as f:
            f.write(result)
